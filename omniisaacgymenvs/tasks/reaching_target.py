...
class BasicReachingTargetTask(RLTask):
    def __init__(self, name, sim_config, env, offset=None) -> None:
        ...
        self._num_observations = 1 + 3 + 6 + 6 + 3 + 4 + 3
        '''
    ...
    def get_observations(self) -> dict:
        ...
        # goal_coord = o3d.geometry.TriangleMesh().create_sphere(radius=0.01)
        # goal_coord.paint_uniform_color([0, 1, 0])
        # goal_coord.translate(goal_pos_np)

        # o3d.visualization.draw_geometries([transformed_point_cloud, tool_tip_position, base_coord, tool_coord, goal_coord],
        ...
        self.obs_buf = torch.cat((
                                  torch.unsqueeze(self.progress_buf / self._max_episode_length, 1), # [NE, 1]
                                  self.tool_end_point,                          # [NE, 3], tool position by point cloud
                                  dof_pos_scaled,                               # [NE, 6]
    ...
    def reset_idx(self, env_ids) -> None:
        ...
        coord_min = torch.tensor([self.x_min+0.1, self.y_min+0.1, self.z_min+0.1], device=self._device)
        coord_max = torch.tensor([self.x_max-0.1, self.y_max-0.1, self.z_max-0.1], device=self._device)
    ...
    def calculate_metrics(self) -> None:
        ...
        tool_goal_distance_reward = self.relu(-(cur_t_g_d - init_t_g_d)/(init_t_g_d*0.9))

        self.completion_reward = torch.zeros(self._num_envs).to(self._device)
        self.completion_reward[self.current_tool_goal_distance<0.045] = 20.0
        self.rew_buf[:] = tool_goal_distance_reward + self.completion_reward
        # print(f'reward: {self.rew_buf.cpu().numpy()}')
    ...
    def is_done(self) -> None:
        ...
        reset = torch.where(self.tool_end_point[:, 0] < self.x_min, ones, reset)
        reset = torch.where(self.tool_end_point[:, 1] < self.y_min, ones, reset)
        reset = torch.where(self.tool_end_point[:, 0] > self.x_max, ones, reset)
        reset = torch.where(self.tool_end_point[:, 1] > self.y_max, ones, reset)
        reset = torch.where(self.tool_end_point[:, 2] < self.z_min, ones, reset)
        reset = torch.where(self.tool_end_point[:, 2] > self.z_max, ones, reset)


        # target reached
        reset = torch.where(self.current_tool_goal_distance <= 0.04, ones, reset)
